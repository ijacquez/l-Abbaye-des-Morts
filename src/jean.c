...
static const tile_rect_t _jean_walk_frames[] = {
    { 384, 88, 16, 24 },
    { 400, 88, 16, 24 }
};

static const tile_rect_t _jean_jump_frames[] = {
    { 416, 88, 16, 24 }
};

static const tile_rect_t _jean_death_frames[] = {
    { 432, 88, 16, 24 },
    { 520, 88, 16, 24 },
    { 536, 88, 16, 24 }
};

static const tile_rect_t _jean_ducking_frames[] = {
    { 482, 88, 20, 24 },
    { 502, 88, 20, 24 }
};
...
void movejean(struct hero *jean)
{
    ...
    if (jean->jump == 1) {
        if (jean->height == 0) {
            sound_mgr_sfx_play(SFX_TYPE_JUMP);
        }
...
void drawjean(struct hero *jean, int counter[])
{
    sprite_t jean_sprite;

    jean_sprite.x = jean->x;
    jean_sprite.y = jean->y;
    jean_sprite.tile_rect = &_jean_walk_frames[0];
    jean_sprite.flip = SPRITE_FLIP_HORIZONTAL * (1 - jean->direction);

    if (jean->death == 0) {
        int index = (jean->animation / 7) % 2;

        if (jean->jump > 0) {
            jean->animation = 0;

            jean_sprite.tile_rect = &_jean_jump_frames[0];
        } else if (jean->ducking == 0) {
            jean_sprite.tile_rect = &_jean_walk_frames[index];
        } else {
            jean_sprite.y += 11;

            jean_sprite.tile_rect = &_jean_ducking_frames[index];
        }
    } else {
        jean->death++;
        ...
        if (jean->death == 2) {
            sound_mgr_sfx_play(SFX_TYPE_ONEDEATHN);
        }

        /* Needs major refactoring. This is an attempt to do an animation */
        if ( (jean->death <  8) ||
                ((jean->death > 23) && (jean->death < 32)) ||
                ((jean->death > 47) && (jean->death < 56))) {
            jean_sprite.tile_rect = &_jean_death_frames[0];
        }

        if (((jean->death >  7) && (jean->death < 16)) ||
                ((jean->death > 31) && (jean->death < 40)) ||
                ((jean->death > 55) && (jean->death < 64))) {
            jean_sprite.tile_rect = &_jean_death_frames[1];
        }

        if (((jean->death > 15) && (jean->death < 24)) ||
                ((jean->death > 39) && (jean->death < 48)) ||
                ((jean->death > 63) && (jean->death < 73))) {
            jean_sprite.tile_rect = &_jean_death_frames[2];
        }
    }
    ...
    render_mgr_sprite_render(&jean_sprite);
}
    ...
    if (y > 0) {
        ...
        if (room[0] == 2) {
            if (((stagedata[room[0]][y + 1][x] > 300) &&  (stagedata[room[0]][y + 1][x] < 305)) || ((stagedata[room[0]][y + 1][x + 1] > 300) &&  (stagedata[room[0]][y + 1][x + 1] < 305))) {
                ...
                jean->flags[1] = 1;
                sound_mgr_bgm_pause();
                sound_mgr_sfx_play(SFX_TYPE_MECHANISMN);
                sleep(2);
