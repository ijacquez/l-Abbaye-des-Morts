...
void drawenemies(struct enem *enemies, SDL_Texture *tiles)
{
            ...
            if (enemies->type[i] == 13) { /* Water movement */
                if (((enemies->speed[i] > 30) && (enemies->speed[i] < 40)) || ((enemies->y[i] == enemies->limright[i] - 16) && (enemies->direction[i] == 1))) {
                    ...
                    if ((enemies->speed[i] > 30) && (enemies->speed[i] < 40)) {
                        sound_mgr_sfx_play(SFX_TYPE_SLASH);
                    }
                }
...
void movenemies(struct enem *enemies, uint stagedata[][22][32], uint counter[], float proyec[], struct hero jean)
{
    int i = 0;
    int tilex = 0;
    int tiley = 0;
            ...
            if (enemies->speed[i] == 50) {
                for (n = 0; n <= 4; n += 2) {
                    if (proyec[n] == 0) {
                        ...
                        n = 5;
                        sound_mgr_sfx_play(SFX_TYPE_SHOOT);
                    }
                }
            }
        }
...
void plants(struct enem *enemies, SDL_Texture *tiles, uint counter[], float proyec[])
{
...
void crusaders(struct enem *enemies, SDL_Texture *tiles, uint counter[], uint room[])
{
...
void death(struct enem *enemies, SDL_Texture *tiles, uint counter[], float proyec[], uint stagedata[][22][32])
{
...
void dragon(struct enem *enemies, SDL_Texture *tiles, uint counter[], float proyec[])
{
...
void satan(struct enem *enemies, SDL_Texture *tiles, uint counter[], float proyec[])
{
...
void fireball(struct enem *enemies, SDL_Texture *tiles, uint counter[], struct hero jean, int stagedata[][22][32])
{
...
void drawshoots(float proyec[], struct enem *enemies)
{
    ...
    for (n = 0; n <= 4; n += 2) {
        if (proyec[n] > 0) {
            ...
            destile.width = srctile.width;
            destile.height = srctile.height;
            ...
            case 0:
                if ((proyec[n] < (enemies->limright[i] - 8)) && (proyec[n] != 0)) {
                    out_x = proyec[n];
                    out_y = enemies->y[i] + 8;
                    render_mgr_tileset_render(&srctile, out_x, out_y);
                    /* SDL_RenderCopy(renderer,tiles,&srctile,&destile); */
                }
